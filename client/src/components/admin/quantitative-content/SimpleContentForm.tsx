...
const SimpleContentForm: React.FC<ContentFormProps> = ({
  ...
  // Watch content value for preview with debug logging
  const watchContent = contentForm.watch("content");
  const watchTitle = contentForm.watch("title");
  
  // Add effect to log when content changes and update preview if needed
  React.useEffect(() => {
    console.log("Content changed in form - preview should update", {
      contentType: typeof watchContent,
      contentLength: typeof watchContent === 'string' ? watchContent.length : 'unknown',
      contentSample: typeof watchContent === 'string' ? watchContent.substring(0, 50) + '...' : JSON.stringify(watchContent).substring(0, 50) + '...'
    });
    
    // If currently in preview mode, force a switch to preview again to refresh content
    if (activeTab === "preview") {
      console.log("Currently in preview tab - refreshing preview");
      // Use setTimeout to ensure this happens after render cycle
      setTimeout(() => {
        // This creates a new key for the RichTextContent component
        setActiveTab("preview");
      }, 10);
    }
  }, [watchContent, activeTab]);
        ...
        {isCreatingTopic || (!isCreatingContent && topic && !content) ? (
          ...
          <Form {...contentForm}>
            <form onSubmit={contentForm.handleSubmit(handleContentSubmit)} className="space-y-4">
              ...
              <div className="pt-2">
                <Tabs defaultValue="edit" value={activeTab} onValueChange={(newTab) => {
                  console.log("Tab changing from", activeTab, "to", newTab);
                  // If switching to preview, ensure we have the latest content
                  if (newTab === "preview") {
                    console.log("Switching to preview tab with content:", typeof watchContent, watchContent?.length);
                  }
                  setActiveTab(newTab);
                }}>
                        ...
                        <ScrollArea className="h-[400px] rounded-md border p-4">
                          <div>
                            <RichTextContent
                              ...
                              key={typeof watchContent === 'string' 
                                ? `preview-string-${watchContent.length}-${Date.now()}` 
                                : `preview-object-${JSON.stringify(watchContent).length}-${Date.now()}`} 
                            />
                          </div>
                        </ScrollArea>
