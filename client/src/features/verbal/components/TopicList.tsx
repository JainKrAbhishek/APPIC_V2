...
import { VerbalTopic } from '@shared/schema';

interface TopicListProps {
  title: string;
  description: string;
  topics: VerbalTopic[] | undefined;
  isLoading: boolean;
...
const TopicList: React.FC<TopicListProps> = ({
  ...
  if (isLoading) {
    return (
      <Card className="h-full shadow-md border-0">
        <CardHeader className="pb-4 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-slate-800 dark:to-slate-700 rounded-t-lg">
  ...
  return (
    <Card className="h-full shadow-md border-0">
      <CardHeader className="pb-2 sm:pb-3 px-3 sm:px-6 pt-3 sm:pt-4 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-slate-800 dark:to-slate-700 rounded-t-lg">
        <div className="flex items-center space-x-2">
          <BookOpen className="h-5 w-5 text-primary" />
          <div>
                ...
                >
                  <div className="absolute left-1.5 xs:left-2 top-1/2 -translate-y-1/2">
                    {userProgress[topic.id] ? (
                      ...
                      <div className={`h-1.5 w-1.5 xs:h-2 xs:w-2 rounded-full ${selectedTopicId === topic.id ? 'bg-white' : 'bg-slate-300 dark:bg-slate-600'}`}></div>
                    )}
                  </div>
                  <div className="flex items-center w-full">
                    <span className="flex-1 text-left truncate font-medium">{topic.title}</span>
                  </div>
                </Button>
              ))
            ) : (
